 ...
 * getTestDictionary() is useful when developing your Checker.
 */

export function getTestDictionary(): Map<string, number> {
    return new Map([
...
export function getLargeDictionary(): Map<string, number> {
    const base = getTestDictionary();
    for (let i = 0; i < 500000; i++) {
        base.set(`xword${i}`, i + 1);
    }
    return base;
}
